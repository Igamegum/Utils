...
void SampleViewer::display()
{
		...
		m_eViewState == DISPLAY_MODE_IMAGE) && m_colorFrame.isValid())
	{
		...
		if(image_index %5 == 0)
		{
			...
			while(temp)
			{
				filename[index_count]  = (temp%10)+'0';
				temp/=10;
				++index_count;
			}

			for(;index_count<5+4;++index_count)  filename[index_count] = '0';
			int p = 0 + 4;
			int q = 5 + 4 - 1;
			while(p<q) {
				char tm = filename[p];
			...
			fprintf(fp,"%d %d\n",m_colorFrame.getHeight(),m_colorFrame.getWidth());
